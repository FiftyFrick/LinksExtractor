...
function formatJD(url, links) {
    ...
    const capitulo = partes.pop(); // 1056
    ...
    return [
        `#packageName: ${nombre}`,
        `#Name: ${nombre}`,
        `#Comment: Extraído automáticamente`,
        "",
        ...links
    ].join("\n");
}
...
app.get("/extract", async (req, res) => {
    ...
    const copy = req.query.copy === "1"; // ?copy=1
    ...
    try {
        ...
        const jdText = formatJD(url, links);

        // 📋 copiar opcional
        if (copy) {
            await clipboard.write(jdText);
        }
